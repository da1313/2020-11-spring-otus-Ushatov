...
public class UserPostListRequestResolverModerated implements UserPostListRequestResolver {
    ...
    public PostByUserListResponse getUserPosts(String userId,  RequestParams params) {
        ...
        String statusNew = blogConfiguration.getModerationStatusList().stream().filter(status -> status.equals(NEW_STATUS))
                .findFirst().orElseThrow(() ->
                        new BadConfigurationException(String.format("Cant find status %s in configuration properties", NEW_STATUS)));

        Page<Post> postPage = postRepository.findByUserIdAndIsActiveAndModerationStatusAndPublicationTimeLessThanEqual(
                PageRequest.of(params.getPageNumber(), params.getPageSize()), userId, true, statusNew, LocalDateTime.now());
    ...
    public UserPostStatus getType() {
        return UserPostStatus.MODERATED;
    }
}
