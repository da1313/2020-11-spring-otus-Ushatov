...
public class UserPostListRequestResolverInactive implements UserPostListRequestResolver {
    ...
    public PostByUserListResponse getUserPosts(String userId, RequestParams params) {
        Page<Post> postPage = postRepository.findByUserIdAndIsActive(PageRequest.of(params.getPageNumber(), params.getPageSize()),
                userId, false);
        List<PostByUser> postByUserList = postPage.getContent().stream().map(postToPostByUserMapper::sourceToDestination)
                .collect(Collectors.toList());
        return new PostByUserListResponse(postByUserList, params.getPageNumber(), postPage.getTotalPages());
    }

    @Override
    public UserPostStatus getType() {
        return UserPostStatus.INACTIVE;
    }
}
