...
import org.course.domain.Permission;
import org.course.domain.Role;
import org.course.security.CustomAccessDeniedHandler;
...
class BookControllerTest {
    ...
    void shouldInvokeDeleteBookMethodAndRedirectToManagePageWithSameNumber() throws Exception {
        ...
        GrantedAuthority authority = new SimpleGrantedAuthority(DbPermissionEnum.BOOK_DELETE.name());

        mockMvc.perform(MockMvcRequestBuilders.post("/manage/book/delete/" + BOOK_ID)
    ...
    void shouldInvokeUpdateBookMethodAndRedirectToManagePageWithSameNumber() throws Exception {
        ...
        GrantedAuthority authority = new SimpleGrantedAuthority(DbPermissionEnum.BOOK_UPDATE.name());

        BookRequest bookRequest = new BookRequest(TITLE, GENRE_IDS, AUTHOR_ID, DESCRIPTION);

        mockMvc.perform(MockMvcRequestBuilders.post("/manage/book/update/" + BOOK_ID)
                .param("title", TITLE)
    ...
    void shouldInvokeCreateBookMethodAndRedirectToManagePageWithSameNumber() throws Exception {
        ...
        GrantedAuthority authority = new SimpleGrantedAuthority(DbPermissionEnum.BOOK_CREATE.name());
